 ...
 * 이름이 없는 좌석은 `null`이다 (화면은 형제 이름으로 대신한다).
 */
export type SeatNameView = {
  seat: BrotherRole
  displayName: string | null
  /**
   * 좌석 **구성**이 봇인지. 봇 구성은 로비에서 이미 공개된 정보다 (`LobbySeatView.isBot`).
   *
   * **봇 대행과 혼동하면 안 된다.** 연결이 끊겨 서버가 대신 조작하는 좌석은 구성이 여전히 인간이라
   * 여기서 `false`다. 그렇지 않으면 화면이 "봇 대행"을 드러내게 된다 (룰북 §17, 아키텍처 §8)
   */
  isBot: boolean
}
