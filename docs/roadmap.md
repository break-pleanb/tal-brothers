...
> 실행 계획은 `docs/m4-plan.md`, 진행 기록은 `docs/m4-notes.md`.

- [x] **착수 전 확정** — 계획 9절 해석 항목 13건 결정, 룰북 §1·§15·§17·§19·§21과 아키 §5.3·§7·§8·§9.1에 반영
- [x] **M4-1 표시 기반** — 승인된 서버·shared 변경 4건과 web 표시 기반
  - [x] `SnapshotMessage.serverNow` — 폰 시계 오차 보정 (9절 2번)
  - [x] `RESOLUTION` 3초 체류 — `GAME_CONFIG.resolutionSeconds`, 타이머 만료에서 다음 단계로 (9절 3번, 룰북 §19)
  - [x] 투영 확장 — 좌석 표시 이름, 현재 진행 번호(총 개수 제외), 엔딩의 배신자 승패·정체 (9절 4번)
  - [x] 개발용 시계 단축 — 개발 환경에서만 받는 방 생성 옵션 (9절 12번)
...
- [x] **M4-4 이벤트 본류**
  ...
  - [x] `CueToast` — 토스트 cue 한 자리 (M4-5의 `WhisperToast`를 앞당겨 통합, 계획 10절 6번의 확인 순서 때문)
  - [x] `PublicView.rollerSeat` — 비공개 판정도 판정자가 직접 굴린다 (계획 9절 14번, 2026-09-20 확정). 은닉 테스트 2건 추가
  - [x] **실기 확인 1차 (8절 1~5번)** — 발견된 4건 수정: 둘째 봇 재굴림 규칙, Display 커서, 좌석 줄 봇 표기, 로비 봇 토글(M4-7 이월분 앞당김)
  - [x] **실기 확인 2차** — Display 멈춤은 서버 결백(재현 스크립트 10개 구성, 버전 누락 0)을 확인하고
        전송 구멍 4가지를 막았다: ws 하트비트, 침묵 감시·재연결, 스냅샷 순서 보장, 연결 상태 표시.
        T2-2 귓속말은 판정자(셋째)에게만 가는 정상 동작이라 고치지 않았다
