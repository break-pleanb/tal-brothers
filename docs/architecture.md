...
| ws 하트비트 | 30초마다 ping, 다음 주기까지 pong이 없으면 소켓을 끊는다. **죽었는데 닫히지 않은 소켓**을 서버가 열려 있다고 믿으면 연결 변화가 일어나지 않아 자동 일시정지도 걸리지 않는다 |
| 클라이언트 침묵 감시 | 단계 마감이 5초 넘게 지났는데 새 스냅샷이 없으면 소켓을 다시 연다. 마감이 없는 단계(로비·일시정지·엔딩)에서는 하지 않는다 |
