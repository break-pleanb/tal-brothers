...
| 자동 저장 | Phase 전환 시 Supabase에 상태 JSON 저장 |
...
| 자동 일시정지 한도 | 게임당 누적 5분. 초과 시 정지하지 않고 계속 진행 |
| 호스트 수동 일시정지 | 이벤트 사이에만 |
...
| 서버 인증 | `session.hello`의 토큰을 Supabase JWT로 검증. 토큰을 URL에 싣지 않는다 |
