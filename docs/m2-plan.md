...
- 룰북에 수치가 없는 항목은 만들지 않는다. 10절에 모아 두고 답을 받았으며, **승인·수정 결과가 10절에 반영돼 있다**
...
| 파일 | 구분 | 핵심 내용 |
|---|---|---|
| `src/sim/humanPolicy.ts` | 생성 | M1 `playPhase1.ts`의 인간 좌석 랜덤 정책을 공용 모듈로 추출 + **가상 입력 지연**(10절 12번) |
...
| `talismanOverflow` | `number` | §9.1 | 상한 2를 넘겨 받은 초과분을 담는 **보류함**. 판정 보정·회복 어느 용도로도 쓸 수 없다 (10절 2번) |
...
| 가짜 흉/평/길 라벨 | `currentEvent.fakeLabels[seat]` | 이벤트 진입 시. **좌석마다 1회** 확률 30% | 걸리면 그 이벤트의 **모든 선택지** 라벨을 가짜로 만들고 통째로 확정 저장. 각 선택지의 가짜 값은 **그 선택지의 실제 변이를 제외한 나머지 두 값 중 무작위 1개** (10절 7번) |
...
```
-1. 보류함(talismanOverflow) 정리 — 남아 있으면 모두 버린다 (10절 2번)
0. 타겟 지정
   후보 = 잠식도 ≥ 30%인 좌석(봇 포함)
...
- `VOTING`이 `talisman.transfer` / `talisman.discard`를 받고, **마감 시점에 남은 보류함 부적을 자동으로 버린다** (10절 2번)
- `P3_TARGETING` 진입 시 보류함에 남은 부적을 모두 버린다. Phase 3까지 들고 가지 않는다 (10절 2번)
...
```
npm run sim:bots -w tal-brothers-server -- --games 300 --seed 1            # 기본 --humans 3
npm run sim:bots -w tal-brothers-server -- --games 300 --seed 1 --humans 0 # 투표 3분 만료 상한 참고치
```

- `--humans` 기본값은 **3**이고 0~3을 받는다. 인간 좌석은 시뮬레이터의 가상 입력 지연 정책으로 채운다 (10절 12번)
...
- **구성별(인간 0~3명) 평균 소요 시간과 타임오버 비율을 나눠 싣는다.** 인간 0명 구성은 투표가 항상 3분 만료라 시계 소모의 **상한 참고치**이고, 조기 마감이 섞인 값은 `--humans 1~3`으로 읽는다 (10절 12번)
- 인간 좌석은 가상 입력 지연을 거쳐 명령을 보내므로, 조기 마감이 0초가 아니라 실제 체감에 가까운 시각에 일어난다
...
## 10. 해석 항목과 결정 (2026-09-18 확정)

> 룰북·아키텍처에 없어 임의로 정할 수 없던 항목이다. 아래 결정을 받아 M2에 착수한다.
> **승인**은 계획의 가정을 그대로 쓰고, **수정**은 아래 소절의 규칙으로 대체한다.

| # | 항목 | 가정 | 결정 |
|---|---|---|---|
| 1 | Phase 2 진입 환청을 단계로 둘지 이벤트로 둘지 | `PHASE2_ENTRY` 단계를 새로 만든다. 선택지가 없고 좌석마다 결과가 달라 정적 효과 목록으로 표현할 수 없다 | **승인** |
| 2 | 부적 보유 상한 초과 처리 | 초과분을 `talismanOverflow`에 두고 다음 투표 시간 중에 처리한다 | **수정 — 10.1** |
| 3 | 14A 미제출 시 "무작위 1명"의 모집단 | 좌석 전원(봇·배신자 포함). 배신자는 100% 고정이라 실효가 없다 | **승인** |
| 4 | 티어 환청 대상이 배신자일 때의 구간 표기 | 배신자는 100% 구간으로 말한다. 진실/거짓 규칙은 그대로 | **승인** |
| 5 | 같은 이벤트에서 환청이 여러 건 겹칠 때 | 좌석당 이벤트당 티어 환청 1건. 이벤트 귓속말(02A 등)과는 겹칠 수 있다 | **승인** |
| 6 | 가짜 붉은 메시지의 발동 시점 | 랜덤 이벤트 **시작 시** 60~99% 좌석마다 굴린다 | **승인** |
| 7 | 가짜 라벨이 붙는 범위 | 좌석마다 굴리고, 걸리면 그 이벤트의 **모든 선택지** 라벨을 가짜로 만든다 | **승인 + 보강 — 10.2** |
| 8 | 13번 발목 대상의 기계적 효과 | 연출 전용. 판정자·효과 대상과 무관하다 | **승인** |
| 9 | 보스 실패 "추격받으며 통과" | 서사 전용. 추가 효과 없음 | **승인** |
| 10 | 봇 100% 방해의 발동 시점 | 봇이 100%에 도달한 직후 즉시. 디버프는 다음 판정에, 부적 소멸은 그 자리에서 | **승인** |
| 11 | 1인 플레이의 "전원 배신자" 검사 | 인간 2명 이상일 때만 검사한다 | **승인** |
| 12 | 봇 자동 대전의 투표 마감 | 룰 그대로 3분을 흘리고, 조기 마감이 섞인 값은 `--humans 1~3`으로 따로 본다 | **수정 — 10.3** |
| 13 | 세이브 재현을 위한 난수 상태 | 시뮬레이터가 rng 인스턴스를 유지한다. 상태 저장은 M5에서 결정한다 | **승인** |

### 10.1 [수정] 부적 보유 상한 초과 처리 (2번)

초과분은 인벤토리가 아니라 **보류함(`talismanOverflow`)** 에 둔다.

| 항목 | 규칙 |
|---|---|
| 사용 | 보류함의 부적은 **판정 보정·회복 어느 용도로도 쓸 수 없다.** 개입 창 2단계, `talisman.heal`, 14A 제출에서 모두 세지 않는다 |
| 처리 시점 | **다음 투표 시간 중**에 `talisman.transfer`(받는 좌석의 `talismanCount`가 상한 미만일 때만) 또는 `talisman.discard` |
| 미처리 | 그 투표가 **마감될 때** 남아 있으면 자동으로 버린다 |
| Phase 3 | 보류함을 들고 Phase 3에 들어가지 않는다. `P3_TARGETING` 진입 시 남아 있으면 모두 버린다 |

- 이유: 초과분을 계속 들고 다니면 사용 가능 여부가 모호해진다. 새 단계나 새 설정값을 만들지 않는다는 원래 의도는 유지된다
- 봇 좌석은 명령을 보내지 않으므로 보류함이 생기면 투표 마감 때 자동으로 버려진다
- 양도 알림은 받는 좌석에만 보낸다. Display에는 표시하지 않는다 (룰북 §21)

### 10.2 [승인 + 보강] 가짜 라벨의 값 규칙 (7번)

좌석마다 1회 굴리고(`tier60FakeLabelChance` 30%), 걸리면 그 이벤트의 **모든 선택지**를 가짜로 표시한다.
여기에 값 규칙을 더한다.

> 가짜 라벨의 각 선택지 값은 **그 선택지의 실제 변이를 제외한 나머지 두 값 중 무작위 1개**다.

- T2 귓속말(룰북 §12)·티어 환청(§16)의 거짓 규칙과 같은 방식이다. 거짓 정보의 생성 규칙을 하나로 통일한다
- 선택지마다 값을 따로 굴리지만, **발동 여부는 좌석당 1회**다. 굴림 결과는 이벤트 진입 시 확정해 저장한다
- 셋째의 절대 시야와 배신자의 열람은 가짜 라벨을 무시하고 항상 진짜를 본다 (룰북 §3.4, §10.2)

### 10.3 [수정] 시뮬레이터의 가상 입력 지연과 봇 자동 대전 구성 (12번)

M1 노트의 제안(인간 입력이 단계 진입 시각에 한꺼번에 도착해 조기 마감이 항상 0초로 끝난다)을 M2에서 함께 해결한다.
M2의 목적이 수치 검증이므로, 시계 소모가 과소 평가되면 타임오버 비율과 우회 페널티의 체감을 측정할 수 없다.

| 항목 | 규칙 |
|---|---|
| 입력 지연 | 인간 좌석은 단계 진입 즉시가 아니라 **지연 뒤에** 명령을 보낸다. 가상 시계를 그 시각까지 진행시킨 뒤 명령을 넣는다 |
| 투표 | 투표 시간(3분) 안에서 무작위 지연 후 제출 |
| 굴림·개입 | 각 창 안에서 짧은 무작위 지연 후 입력 |
| 설정 | 지연 분포는 `humanPolicy.ts`의 설정 가능한 값으로 둔다. 시뮬레이터 전용이며 **룰북은 건드리지 않는다** |
| CLI | `sim:bots`는 `--humans`를 받고 **기본값 3**. 인간 0명 구성은 투표 3분이 그대로 흐르는 상한 참고치로만 따로 실행한다 |
| 지표 | 8.2에 구성별(인간 0~3명) 평균 소요 시간과 타임오버 비율을 구분해 싣는다 |

**지연 분포 제안** (모두 시뮬레이터 설정값)

| 입력 | 분포 | 근거 |
|---|---|---|
| 투표 제출 | 15~120초 균등 | 상황 제시 후 토론하다 투표 시간(180초) 안에 누른다. 상한을 120초로 두어 3분 만료와 조기 마감이 모두 나온다 |
| 절대 시야·부적 회복 | 5~30초 균등 | 투표 전에 정보를 먼저 확인한다 |
| 주사위 굴림 | 1~6초 균등 | 자동 굴림(10초)보다 앞선다 |
| 개입 입력 | 0.5~3초 균등 | 각 단계 4초 안에 들어간다 |
| 보류함 처리 | 10~60초 균등 | 투표 시간 중 처리 (10.1) |

## 11. 진행 순서와 확인 방법

1. **10절 항목 확인** → 2026-09-18 확정. 승인 10건, 수정 2건(2번·12번), 승인+보강 1건(7번)을 10절에 반영했다
...
10. 작업 단위마다 `M2-N: 요약` 형식으로 커밋한다. push는 하지 않는다
